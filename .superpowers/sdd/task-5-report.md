...
## Same-Thread Resume and Stable Action Ordering (2026-07-09)

- `_accepted_for_thread` 不再选择同 thread 的第一条历史 Action；它只收集 `pending / approved / rejected` 活跃 Action，零条明确报告 inactive，多条抛出冲突，且仅在恰好一条时恢复对应 Attempt。
- rollover 在锁定旧 Action 的同一事务中读取该 Attempt 的最大 Action `created_at`，并以 `max(clock, max_created_at + 0.001)` 写入新 Action；SQLite 与 PostgreSQL 共用同一 primitive，Timeline 继续按 `created_at, id` 获得稳定业务顺序。
- TDD RED：冻结时钟多次 rollover 顺序错误、同 thread 第二次 resume 命中 completed Action、重复活跃 Action 未冲突、inactive Action 仍被接受、PostgreSQL 契约缺少最大时间；修复后五项全部转绿。
- Ordering focused 连续三次：每次 `76 passed`。
- Expanded focused：`120 passed in 27.94s`。
- Full：`883 passed, 6 skipped in 69.13s`；六项 skip 仍为可选 `customer_band` provider 未安装。
- Static：scoped Ruff check、Ruff format check、`git diff --check` 全部通过。
