...
- The trusted server-derived Action key takes precedence over legacy Skill-authored keys during approval resume. Calls without a trusted approval key retain their previous explicit-key behavior.
...
## Trusted-Key Compatibility Regression

- Correct-project `.venv` testing exposed that `refund.apply` supplies a legacy Skill key (`refund:user:order`). Treating it as a conflict with the trusted Action key caused an otherwise valid approved refund to end as `failed`.
- The precedence rule is now explicit: during trusted Action resume, `ToolExecutor` deterministically overwrites any Skill-authored key with the server-derived Action/tool/business-args key. Outside trusted approval context the Skill key is unchanged.
- RED: `test_side_effect_pauses_then_resume_completes` returned `failed`; the focused precedence test raised `IdempotencyConflictError`. GREEN: both passed, expanded focused verification reported `65 passed`, and the correct-project `.venv` full run reported `910 passed, 6 skipped in 74.39s`.
- Correct-project `.venv` Ruff lint, scoped Ruff format, scoped mypy, and `git diff --check` passed after formatting.
