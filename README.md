...
agentkit/
  src/agentkit/                       # installable runtime package
    ...
    core/                             # generic agent platform runtime
      ...
      input_resolution.py             # schema-driven slots, confidence, defaults, clarification
...
- RAG 入库、检索与评估：[`docs/RAG_WORKFLOW.md`](docs/RAG_WORKFLOW.md)
...
- `actions_enabled: true`：行动型 agent。仍然是 chat 交互且有同一套会话记忆，但会进入 `AgentGateway.handle → EnterpriseAgentGraph`，完整经过 intent/route/input-resolution/plan/review/approval/execution/output-review/audit。调用方需要 `chat:use` + `task:run`；审批通过 `context.approval` 进入同一 `/api/chat*` 入口并额外要求 `task:approve`。
...
- **自然语言入参解析**：路由选定 skill 后，`SkillInputResolver` 按 `skill_args → request.context → IntentFrame.entities → LLM 语义抽取 → schema default` 的优先级生成参数。用户不需要遵循固定句式；只有无法可靠获得 schema `required` 字段时才返回 `output.status=needs_clarification`，且不会进入审批或工具执行。浏览器会保留已解析参数，下一句可直接补充缺失值。
- **可追溯性**：审计事件 `skill_inputs_resolved` 记录每个 slot 的值、来源、置信度和默认原因；`input_clarification_requested` 记录缺失字段。schema 根节点可设置 `x-agentkit-infer-from-message: true`，要求解析可选字段中的显式自然语言信息；字段可用 `x-agentkit-label` 提供澄清文案标签。租户可通过 `prompts["nodes.input_resolution"]` 覆盖该节点提示词。
...
审批前默认有 5 次串行治理 LLM 调用(intent/route/plan/plan_review/approval-assessment),在 1rps 限速下是耗时主因。开启 `AGENTKIT_DETERMINISTIC_FASTPATH=true` 后:当**规则路由**能以 `confidence=high` 命中某个 skill(例如 "Rank candidate C-100 for JOB-001" 经 `routing_hints` 命中 `candidate.rank`)时,**跳过这 5 次治理 LLM**。若 schema 必填参数已由结构化上下文/规则实体补齐，则输入解析也是 0 次 LLM；若路由明确但参数只能从自由文本理解，则仅增加 1 次结构化 slot 抽取。抽取后仍缺必填参数时返回澄清，不进入审批。审计会记录 `fastpath_engaged` 与 `skill_inputs_resolved`。默认关闭,确保治理可见性不被悄悄削弱——需要更低延迟时显式开启。
...
两者关系:fast-path 处理“路由与参数都可确定”的请求(**0 次** LLM)，或“路由确定、参数需语义抽取”的请求(**1 次** LLM)；合并节点处理“路由必须走 LLM”的请求(intent+route 从 2 次降到 **1 次**);fast-path 优先级更高。
...
- `marketing.social_growth`: runs a multi-skill Xiaohongshu workflow through
  ...
  comparison, strategy planning, copy generation, machine review, frozen-content
  human approval, direct publishing, and KPI tracking.
...
5. The input resolver maps free-form language and trusted structured context to
   the selected skill's JSON Schema. Missing required fields return a
   `needs_clarification` response before approval or execution.
6. The planner creates the execution plan. Platform validation preserves hard
   constraints such as batch execution for configured batch thresholds.
7. The plan reviewer validates the plan before execution.
8. The human approval gate checks tenant policy and, when needed, asks the LLM
...
9. The executor checks policy, runs the skill in batches, calls mock tools, and records audit events.
10. The LLM output reviewer validates the result before finalization.
11. The LangGraph agent finalizes the response.
12. The response contains ranked candidates plus the generated plan, governance metadata, and audit trail.
...
- `core/router.py`: route selection into a registered skill or no business
  route.
- `core/input_resolution.py`: schema-driven skill argument extraction,
  confidence checks, defaults, and clarification.
...
`xhs.metrics.track`), and RPA-style tools for research, package preparation,
approved direct publishing, and metrics. The tenant config grants the matching permissions via the
`growth_manager` role.
...
Live Xiaohongshu research is available through the optional reusable
Playwright connector. The default remains `mock` so offline development and CI
do not require a browser. Install and authenticate once, then enable it:

```bash
uv sync --extra browser
uv run playwright install chromium
uv run agentkit --tenant company_alpha browser-login xhs
uv run agentkit --tenant company_alpha browser-login xhs --target publish
# .env
...
configuration, diagnostics, and the adapter contract for adding other sites.

Direct publication uses a two-phase checkpoint. The first phase generates the
article and cover, runs deterministic plus LLM review, freezes the exact
title/body/tags/media under a SHA-256 content fingerprint, and returns
`waiting_for_approval`. The approval UI shows that frozen content. `Approve &
Publish` resumes the same checkpoint, verifies the approved fingerprint, and
calls `xhs.rpa.publish_note` without rerunning research or generation. The
publisher has a durable SQLite idempotency ledger and does not automatically
retry browser submission; an uncertain post-click outcome requires manual
