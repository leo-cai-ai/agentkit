...
agentkit/
  ...
  data/
    <tenant>.sqlite                   # per-tenant SQLite persistence files
  tenants/
...
回答型 agent 的上下文按 token 预算组装：`persona + 检索到的长期记忆 + 摘要(summary) + 最近几轮原文 + 当前问题`。行动型 agent 在进入 LangGraph 前会读取同一条 conversation 的 `summary/recent_messages/retrieved_memories` 并写入 `TaskRequest.context.chat_memory`，执行结束后把用户消息和最终回复写回 conversation store。所有消息整段持久化到每租户 SQLite；每隔 N 轮可用 LLM 抽取「持久事实」存入向量库，后续按余弦相似度检索（去重 + 最低分阈值）。
...
CLI and Flask console runs are both persisted to per-tenant SQLite files at:
...
intent/route/plan. `AGENTKIT_APPROVAL_CHECKPOINTER=sqlite` persists those
checkpoints at `data/<tenant>_checkpoints.sqlite` for multi-worker or restart
resilience.
