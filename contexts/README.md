...
- `business/`：候选人摘要、小红书文章生成与内容审核等业务 LLM 节点。每个 Pack 必须通过
  `owner_skill` 指向根目录 `skills/` 中的能力包。
- `fragments/`：Runtime 强制注入的安全、不可信数据、证据与 JSON 输出规则。
- `overrides/<tenant-selector>/`：租户模板覆盖；只允许 `system.md` 和 `user.md`。

根目录 `skills/` 是完整、可移植的业务能力包，保存 `SKILL.md`、`skill.yaml`、脚本、Workflow
与 Tool；`contexts/business/` 只保存其中某个 LLM 调用节点的上下文契约。两者不是镜像目录，
不得把完整 Skill 流程和规则复制到 Context Pack。

当前共 15 个 Pack：11 个 Runtime Pack、4 个 Business Pack。每个 Pack 必须声明 `context.yaml`、System/User 模板、输入 Source 白名单、Token 预算和输出模式；
JSON 输出还必须提供严格 Schema。`ContextInvocationService` 会在调用模型前把 Registry 中经过校验的
Schema 注入可信 System 上下文，并把这部分内容计入输入 Token 预算；业务 Prompt 不需要重复复制
Schema。动态输入不会进入 System Message，未声明输入不会被渲染。
