...
命令会打开有界面的 Chromium，并使用配置的持久 profile。人工完成扫码登录或验证码，
确认搜索结果已显示后回到终端按 Enter。AgentKit 不自动识别或绕过验证码。
...
发布页面出现登录或验证码时立即停止。AgentKit 不尝试绕过 CAPTCHA；重新运行
`agentkit --tenant company_alpha browser-login xhs --target publish`，在同一 profile 中人工完成验证。
