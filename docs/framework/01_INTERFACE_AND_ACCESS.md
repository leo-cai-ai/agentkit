...
| 恢复审批 | `/api/tasks/resume` 或 Chat 的 `approval` | 从原 Checkpoint 继续，而不是创建重复副作用 |
...
客户端断开时会设置取消标志并停止继续投递流事件，但这不等价于通用业务任务取消协议；已经执行的外部副作用不会因浏览器关闭而自动回滚。
...
| 类别 | 示例 | 表达方式 |
|---|---|---|
| 接口输入错误 | 未启用 Agent、缺少 `thread_id` | HTTP 400 + `{"error":"..."}` |
| 未认证/无权限 | 未登录、缺少平台权限 | 302/401/403/503，取决于认证状态 |
| CSRF 失败 | 状态修改请求缺 token | HTTP 400 |
| 资源不存在 | 会话不存在或不属于当前用户 | HTTP 404 或 Chat 输入错误 |
...
| 关注点 | 源码 |
|---|---|
| Flask 路由和请求转换 | [`src/agentkit/web/app.py`](../../src/agentkit/web/app.py) 中的 `_task_request`、`_run_chat`、`_run_task`、`_resume` |
...
- [`tests/integration/test_chat_api.py`](../../tests/integration/test_chat_api.py)：Chat/Task、会话、重试、删除和审批 API。
...
回答时可以引用 `_chat_task_request` 强制 General 会话、`_task_request` 忽略自报角色、`stream_response` 的三类事件，以及 Chat/Task 最终共用 `AgentGateway`。
