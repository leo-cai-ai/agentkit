...
| --- | --- | --- | --- | --- | --- |
| 近期消息 | 当前会话原始消息 | 会话级 | `conversation_id` | 否 | 无法恢复短期上下文 |
...
`ConversationContextService` 按 `window_turns * 2` 读取近期消息，因为一轮通常包含 User 和 Assistant 两条消息。Assistant 历史先通过 `normalize_persisted_assistant_text()` 规范化，避免把旧的内部结构或错误显示重新注入模型。
...
3. 找出上次摘要尚未覆盖、且已滑出近期窗口的消息。
4. 调用 `runtime.memory-summary` Context Pack 增量折叠。
...
`ConversationPersistenceService.record_turn()` 在写入前再次验证：

- `conversation_id` 存在。
- 会话同时属于当前 `tenant_id`、所有者 `agent_id` 和 `user_id`。
- 会话状态为 `active`。

Assistant 文本会移除隐藏推理标签后再保存。委派轮次可以用 `assistant_agent_id` 标记实际回答者，同时仍写回 General 所有的会话。

重试使用 `retry_of_run_id` 尝试原位替换失败轮次，避免在历史里重复堆叠同一个 User 请求。未找到可替换轮次时才追加消息，并记录 `conversation_retry_replace_missed`。
...
- [`tests/unit/test_conversation_persistence.py`](../../tests/unit/test_conversation_persistence.py)：写回、重试替换、摘要和失败隔离。
...
> AgentKit 没有把 Memory 等同于聊天记录。我们分开管理近期消息、增量摘要、按租户/Agent/用户隔离的长期语义 Memory、带 Collection 与 ACL 的企业 RAG，以及只服务于执行追溯的 Artifact。委派时共享 General 会话主线，但目标 Agent 只读取自己的长期 Memory 和知识域。Memory 写回采用最终一致的辅助能力，失败不回滚业务；RAG 采用混合检索、可选 LLM 改写/重排和 Context Token 门禁。OCR 是 XHS 与 RAG 共用 Provider，`none` 是不发网络请求的硬关闭状态。
