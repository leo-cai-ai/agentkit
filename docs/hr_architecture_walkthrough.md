...
```json
"approval_required_skills": ["xhs.publish.prepare", "candidate.rank"]
```

HR 排名默认需要人工审批，所以可以在控制台测试 checkpoint/resume。
`xhs.growth.campaign` 不在这里，是因为它先生成并 review 冻结内容，再在
`post_execution_approval` 做二阶段发布审批；它不会绕过人工审批。
...
`ui` 中的 HR 参数只用于示例文案/展示，不会再静默注入业务执行：
...
`src/agentkit/web/app.py` 会把行动型 agent 转成 `TaskRequest`，只保留调用方明确传入的业务字段，
不会从 tenant UI 默认值补齐 `job_id`、`candidate_ids`、`top_n`。自然语言里显式写出的
`JOB-001`、`C-100` 等会被 intent 层抽取；路由后 `SkillInputResolver` 再按照
`candidate.rank.input_schema` 检查必填参数。缺少 `job_id` 或 `candidate_ids` 时返回
`needs_clarification`，不会进入审批和 ATS 工具。用户下一句可直接补充缺失值。
