...
不直接修改仓库共享 `.venv`。在 worktree 内创建临时升级环境，通过 `UV_PROJECT_ENVIRONMENT` 指向该环境并执行同步、检查和测试。验证完成后删除该临时环境，删除前必须确认绝对路径位于当前 worktree 内。
...
现有 `uv.lock` 和升级前提交构成回退点，不对共享环境执行不可逆变更。
