"""SQLite-backed conversation persistence (per tenant).

Shares the per-tenant database file with the audit log but owns its own tables
(``conversations`` / ``messages`` / ``conversation_summaries``). All chat turns
are persisted in full; the rolling summary only exists to save context tokens.
