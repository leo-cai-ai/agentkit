...
def test_xhs_approval_preview_renders_each_text_image_page(client) -> None:
    js = client.get("/static/js/app.js").get_data(as_text=True)

    assert "preview.card_pages" in js
    assert "approval-card-page" in js
    assert "preview.card_text" not in js
